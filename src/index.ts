export function getBlobByUrl(url: string | URL, detectFileName = false) {
  return new Promise((resolve: (blob: { blob: Blob; filename: string }) => void, reject) => {
    ...
    xhr.onload = () => {
      ...
      if (detectFileName) {
        ...
        // try {
        const contentDispostion = xhr.getResponseHeader('content-disposition');
        // } catch (error) {
        //   contentDispostion = null;
        // }
        // console.log('contentDispostion', contentDispostion);
        if (contentDispostion && contentDispostion.match(/filename\*?=(utf-8'')?("?)([^"]*)\2$/)) {
